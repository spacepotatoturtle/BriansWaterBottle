...
@Autonomous(group = "Performance", name = "AutoE")
public class AutoE extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        double powerShotRPM = 1500;
        double[] powerShotAngles = {0, 6 * Math.PI / 180, -11 * Math.PI / 180};

        Pose2d startingPose = new Pose2d(9, -39, 0);
        Pose2d powerPose = new Pose2d(64, -67, 0);
        Pose2d powerPoseFinal = new Pose2d(64, -67, -5 * Math.PI / 180);
        Pose2d pickupPlace1 = new Pose2d(127, -35, -85 * Math.PI / 180);
        Pose2d pickupPlace2 = new Pose2d(129, -70, -85 * Math.PI / 180);
        Pose2d zonePose;
        Pose2d wobblePose = new Pose2d(46, -32, 0);  // 43, -19, Math.PI / 6
        Pose2d wobblePose2 = new Pose2d(35, -34, 0);  // 37, -24, Math.PI / 6
        Pose2d shootingPosefinal = new Pose2d(64, -48, 5 * Math.PI / 180);
        Pose2d zonePose2;
        ...
        numRings = 0; //NOTICE THIS LINE IF WE ACTUALLY WANT TO RUN FOR ANYTHING OTHER THAN 1 ring

        if (isStopRequested()) return;

        if (numRings == 0) {
            zonePose = new Pose2d(90, -30, 0);
            zonePose2 = new Pose2d(80, -32, 0);
        } else if (numRings == 1) {
        ...
        Trajectory toPickup1 = drive.trajectoryBuilder(powerPoseFinal)
                .splineTo(new Vector2d(pickupPlace1.getX(), pickupPlace1.getY()), pickupPlace1.getHeading())
                .splineTo(new Vector2d(pickupPlace2.getX(), pickupPlace2.getY()), pickupPlace2.getHeading())
                .addTemporalMarker(0.1, () -> intake.intakeSpeed(1))
                .splineTo(new Vector2d(zonePose.getX(), zonePose.getY()), -zonePose.getHeading())
        ...
        Trajectory toShootingFinal = drive.trajectoryBuilder(wobblePose2)
                .splineTo(new Vector2d(shootingPosefinal.getX(), shootingPosefinal.getY()), shootingPosefinal.getHeading())
                .build();

        Trajectory toZone1 = drive.trajectoryBuilder(shootingPosefinal)
        ...
        drive.followTrajectory(toShootingFinal);
        drive.update();

        for (int i = 0; i < 3; i++) {

            int county = 0; //tries to rev up for 0.5 seconds before just giving up and shooting
            while ((robot.shooter0.getVelocity() < 0.95 * tempPower || robot.shooter0.getVelocity() > 1.05 * tempPower) && (county < 20)) {
                sleep(25);
                county++;
            }
            shooter.poke();
